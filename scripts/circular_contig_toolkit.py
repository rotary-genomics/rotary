...
def summarize_assembly_info(assembly_info_filepath, contig_list_filepath=None, bed_filepath=None,
    ...
    if contig_list_filepath is not None:
        logger.debug('Exporting circular contig list')
        circular_contigs['#seq_name'].to_csv(contig_list_filepath, index=False, header=None)
...
def check_circlator_logfile(circlator_logfile):
    ...
    if circlator_info['circularised'].sum() == 0:
        ...
        logger.error('File processing error. # of non-circularized contigs is not 0 but is not > 0 either. Exiting...')
        sys.exit(1)
...
def main(args):
    ...
    circular_list_outfile = args.output_contig_list
    ...
    if (assembly_info_filepath is not None) & (circular_list_outfile is None) & (end_regions_outfile is None):
        logger.error('You specified --assembly_info, but you did not specify an output file ' +
                     '(output_contig_list and/or output_bed_file). Exiting...')
        sys.exit(1)
    ...
    logger.debug('Output contig list filepath: ' + str(circular_list_outfile))
    logger.debug('Output BED filepath of end regions: ' + str(end_regions_outfile))
    ...
    if assembly_info_filepath is not None:
        summarize_assembly_info(assembly_info_filepath, contig_list_filepath=circular_list_outfile,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--output_contig_list', required=False, default=None,
    ...
    parser.add_argument('-l', '--length_threshold', required=False, default=100000,
